...
-  **防翻车得分保护**：自由设定自动提交的正确率（覆盖率）阈值和答题超时保护，自动保存拒绝低分交卷。
...
| max_concurrent | 1 | 同时处理的最大章节数量 (针对单一课) |
| notify_level | Chapter | 向 AI 发送**进度通知**的频率 (`Chapter`=按章节, `Course`=按课程, `None`=完全不通知) |
| ai_model_group | default | **AI 题库模型组** (用于调用 Nekro 模型组完成测验) |
| ai_timeout | 15.0 | **AI 答题超时时间(秒)** (避免网络卡顿导致大模型响应无限挂起) |
| ai_submit_threshold | 80 | **自动提交阈值(%)** (题库命中或AI答出比例 >= 此值才自动提交测验，否则仅保存为草稿) |
